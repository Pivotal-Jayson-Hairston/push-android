...
public class GeofenceEngineTest extends AndroidTestCase {
    ...
    public void testEmptyResponseDataWithOneCurrentlyRegisteredGeofenceWithATimestamp() throws IOException {
        final PCFPushGeofenceResponseData updateData = ModelUtil.getPCFPushGeofenceResponseData(getContext(), "geofence_response_data_empty.json");
        final PCFPushGeofenceMultiplexMap expectedMultiplexMap = new PCFPushGeofenceMultiplexMap();

        final PCFPushGeofenceMultiplex multiplex = new PCFPushGeofenceMultiplex();
        multiplex.setMultiplexId("guid1");
        List<String> list = new ArrayList<>();
        list.add("PCF_7_66");
        multiplex.setRequestIds(list);
        expectedMultiplexMap.put("guid1", multiplex);

        when(store.getCurrentlyRegisteredGeofences()).thenReturn(ONE_ITEM_GEOFENCE_LIST);
        engine.processResponseData(50L, updateData);
        assertRegisterGeofences(expectedMultiplexMap, ONE_ITEM_GEOFENCE_MAP);
...
//        assertRegisterGeofences(expectedMultiplexMap, expectedMap);
//
...
//        when(store.getCurrentlyRegisteredGeofences()).thenReturn(ONE_ITEM_GEOFENCE_LIST);
//        engine.processResponseData(50L, updateData);
//        assertRegisterGeofences(expectedMultiplexMap, EMPTY_GEOFENCE_MAP);
...
//        when(store.getCurrentlyRegisteredGeofences()).thenReturn(ONE_ITEM_GEOFENCE_LIST);
//        engine.processResponseData(50L, updateData);
//        assertRegisterGeofences(expectedMultiplexMap, new PCFPushGeofenceLocationMap());
...
//    }
    ...
    private void assertRegisterGeofences(PCFPushGeofenceMultiplexMap expectedMultiplexMap, PCFPushGeofenceLocationMap expectedGeofences) {
        final ArgumentCaptor<PCFPushGeofenceMultiplexMap> multiplexMapCaptor = ArgumentCaptor.forClass(PCFPushGeofenceMultiplexMap.class);
        final ArgumentCaptor<PCFPushGeofenceLocationMap> geofencesCaptor = ArgumentCaptor.forClass(PCFPushGeofenceLocationMap.class);
        verify(registrar).registerGeofences(multiplexMapCaptor.capture(), geofencesCaptor.capture(), any(PCFPushGeofenceDataList.class));
        assertEquals(expectedGeofences, geofencesCaptor.getValue());

        final PCFPushGeofenceMultiplexMap actualMultiplexMap = multiplexMapCaptor.getValue();
        assertEquals(expectedMultiplexMap.size(), actualMultiplexMap.size());
        for (Map.Entry<String, PCFPushGeofenceMultiplex> entry : expectedMultiplexMap.entrySet()) {
            final PCFPushGeofenceMultiplex expectedMultiplex
        }
